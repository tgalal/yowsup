...
class MediaUploader(WARequest, threading.Thread):
    def __init__(self, jid, accountJid, sourcePath, uploadUrl, resumeOffset = 0, successClbk = None, errorClbk = None, progressCallback = None, async = True, userData=None):
    ...
    def run(self):
        ...
        try:
            ...
            while totalsent < int(filesize):
                ...
                if lastEmit!=status and status!=100 and filesize>12288:
                    if self.progressCallback:
                        self.progressCallback(self.sourcePath, self.jid, uploadUrl, int(status), self.userData)
                lastEmit = status
                stream = stream[buf:]
            ...
            if self.progressCallback:
                self.progressCallback(self.sourcePath, self.jid, uploadUrl, 100, self.userData)
            ...
            if result["url"] is not None:
                if self.successCallback:
                    self.successCallback(sourcePath, self.jid, result["url"], self.userData)
            else:
                logger.exception("uploadUrl: %s, result of uploading media has no url" % uploadUrl)
                if self.errorCallback:
                    self.errorCallback(sourcePath, self.jid, uploadUrl, self.userData)
